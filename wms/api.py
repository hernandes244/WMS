...
from .models import Product, ProductBarcode, Address, Stock, Movement, Receipt, Lot, Order, Picking, PickingItem, Imei
...
@authentication_classes([TokenAuthentication])
@permission_classes([IsAuthenticated])
def addresses(request):return Response({'ok':True,'addresses':list(Address.objects.filter(active=True).order_by('code').values('id','code'))})
@api_view(['GET'])
@authentication_classes([TokenAuthentication])
@permission_classes([IsAuthenticated])
...
@permission_classes([IsAuthenticated])
def receive(request):
    try:receipt=Receipt.objects.get(pk=int(request.data.get('receipt_id')));product=Product.objects.get(pk=int(request.data.get('product_id')),active=True);address=Address.objects.get(pk=int(request.data.get('address_id')),active=True);quantity=parse_qty(request.data.get('quantity'))
...
def inventory(request):
    try:product=Product.objects.get(pk=int(request.data.get('product_id')),active=True);address=Address.objects.get(pk=int(request.data.get('address_id')),active=True);counted=parse_qty(request.data.get('counted_quantity'))
    ...
    with transaction.atomic():
        stock,_=Stock.objects.select_for_update().get_or_create(product=product,address=address,lot=lot,defaults={'quantity':0});old=stock.quantity;delta=counted-old;stock.quantity=counted;stock.save(update_fields=['quantity'])
...
def orders(request):
    data=[]
    for o in Order.objects.filter(status__in=['RECEIVED','RELEASED','PICKING']).order_by('priority','created_at')[:100]:data.append({'id':o.id,'number':o.number,'customer':o.customer,'priority':o.priority,'status':o.status,'items':[{'id':i.id,'product':product_payload(i.product),'quantity':str(i.quantity),'picked':str(i.picked)} for i in o.items.select_related('product')]})
        ...
        for o in orders_q:
            ...
            for oi in o.items.all():
                ...
                for s in stocks:
                    ...
                    if take>0:PickingItem.objects.create(picking=p,order_item=oi,address=s.address,quantity=take);s.reserved+=take;s.save(update_fields=['reserved']);remaining-=take
...
def pickings(request):
    data=[]
    for p in Picking.objects.filter(status__in=['RELEASED','RUNNING']).order_by('created_at')[:50]:data.append({'id':p.id,'number':p.number,'status':p.status,'items':[{'id':i.id,'order':i.order_item.order.number,'product':product_payload(i.order_item.product),'address_id':i.address_id,'address':i.address.code,'quantity':str(i.quantity),'picked':str(i.picked)} for i in p.items.select_related('order_item__order','order_item__product','address')]})
...
def pick(request):
    ...
    with transaction.atomic():
        ...
        if not s or s.quantity-s.blocked<quantity:return Response({'ok':False,'error':'Estoque físico insuficiente no endereço.'},status=409)
        if s.reserved<quantity:return Response({'ok':False,'error':'Reserva de separação insuficiente para este item.'},status=409)
        s.quantity-=quantity;s.reserved-=quantity;s.save(update_fields=['quantity','reserved'])
        item.picked+=quantity;item.save(update_fields=['picked']);oi=item.order_item;oi.picked+=quantity;oi.save(update_fields=['picked'])
